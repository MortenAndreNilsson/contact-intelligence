...
export function getCompany(id: string): CompanyWithStats | null {
  const row = queryOne<CompanyRow & { contact_count: number; avg_score: number | null; last_activity: string | null }>(
    `SELECT c.*,
       (SELECT COUNT(*) FROM contacts WHERE company_id = c.id) AS contact_count,
       (SELECT AVG(CAST(json_extract(a.detail, '$.avgScore') AS REAL))
...
export function listCompanies(opts?: { query?: string; industry?: string; sort?: string }): CompanyWithStats[] {
  let sql = `SELECT c.*,
    (SELECT COUNT(*) FROM contacts WHERE company_id = c.id) AS contact_count,
    (SELECT AVG(CAST(json_extract(a.detail, '$.avgScore') AS REAL))
     FROM activities a WHERE a.company_id = c.id AND a.activity_type = 'survey_completed') AS avg_score,
  ...
  const rows = queryAll<CompanyRow & { contact_count: number; avg_score: number | null; last_activity: string | null }>(sql, Object.keys(params).length > 0 ? params : undefined);
  return rows.map((r) => ({
    ...parseCompany(r),
...
export function updateCompany(id: string, fields: Partial<Pick<Company, "name" | "domain" | "industry" | "size_bucket" | "country" | "notes" | "tags">>): void {
  const sets: string[] = [];
  ...
  run(`UPDATE companies SET ${sets.join(", ")} WHERE id = $id`, params);
}
...
export function getCompanyByDomain(domain: string): CompanyWithStats | null {
  const row = queryOne<{ id: string }>(`SELECT id FROM companies WHERE domain = $domain`, { $domain: domain });
  if (!row) return null;
