...
export function getDashboardStats(): DashboardStats {
  const counts = queryOne<{ totalCompanies: number; totalContacts: number; totalActivities: number }>(
    `SELECT
  ...
  const scoreRow = queryOne<{ avgScore: number | null }>(
    `SELECT AVG(CAST(json_extract(detail, '$.avgScore') AS REAL)) AS avgScore
     FROM activities WHERE activity_type = 'survey_completed'`
  );

  const recentActivity = queryAll<ActivityWithNames>(
    `SELECT a.*,
  ...
  const topCompanies = queryAll<CompanyRow & { contact_count: number; avg_score: number | null; last_activity: string | null }>(
    `SELECT c.*,
